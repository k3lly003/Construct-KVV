...
export const useNegotiation = (bidId: string, initialMessages?: any[]) => {
  ...
  const useHistory = useCallback(() => {
    return useQuery({
      ...
      queryFn: async () => {
        ...
        try {
          const result = await negotiationService.getNegotiationHistory(bidId, authToken);
          return result;
        } catch (error) {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (messageData: CreateNegotiationMessageDTO) => {
      ...
      try {
        const result = await negotiationService.sendNegotiationMessage(messageData, authToken);
        return result;
      } catch (error) {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['negotiationHistory', bidId] });
    },
    onError: (error) => {
  ...
  const sendMessage = useCallback(async (messageData: CreateNegotiationMessageDTO) => {
    if (!isClient) {
