...
import {
  ...
  SellerRequest,
} from "@/app/services/sellerRequestService";
...
export default function SellerRequestsPage() {
  ...
  const handleAction = async (id: string, status: "APPROVED" | "REJECTED") => {
    ...
    try {
      ...
      setActionError(
        status === "APPROVED"
          ? t(dashboardFakes.sellerRequest.approveError)
          : t(dashboardFakes.sellerRequest.rejectError)
      );
    } finally {
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">
        {t(dashboardFakes.sellerRequest.title)}
      </h1>
      <div className="bg-white rounded-2xl shadow-lg p-4 md:p-8 overflow-x-auto">
        {loading ? (
          <div className="text-center py-8 text-gray-500">
            {t(dashboardFakes.sellerRequest.loading)}
          </div>
        ) : error ? (
          <div className="text-center text-red-500 py-8">{error}</div>
        ) : requests.length === 0 ? (
          <div className="text-center text-gray-400 py-8">
            {t(dashboardFakes.sellerRequest.noRequestsFound)}
          </div>
        ) : (
          <Table className="min-w-[900px]">
            <TableHeader>
              <TableRow>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableName)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableEmail)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tablePhone)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableBusinessName)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableBusinessAddress)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableBusinessPhone)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableTaxId)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableStatus)}
                </TableHead>
                <TableHead>
                  {t(dashboardFakes.sellerRequest.tableSubmitted)}
                </TableHead>
                <TableHead className="text-center">
                  {t(dashboardFakes.sellerRequest.tableActions)}
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {requests.map((req) => (
                <TableRow
                  key={req.id}
                  className="hover:bg-blue-50 transition rounded-xl"
                >
                  <TableCell>
                  ...
                  <TableCell>
                    ...
                    >
                      {req.status === "PENDING"
                        ? t(dashboardFakes.sellerRequest.statusPending)
                        : req.status === "APPROVED"
                        ? t(dashboardFakes.sellerRequest.statusApproved)
                    ...
                    <div className="flex gap-2 justify-center">
                      ...
                      >
                        {actionLoading === req.id + "APPROVED"
                          ? t(dashboardFakes.sellerRequest.approving)
                          : t(dashboardFakes.sellerRequest.approve)}
                      </button>
                      <button
                      ...
                      >
                        {actionLoading === req.id + "REJECTED"
                          ? t(dashboardFakes.sellerRequest.rejecting)
                          : t(dashboardFakes.sellerRequest.reject)}
                      </button>
                    </div>
                    {actionError && (
