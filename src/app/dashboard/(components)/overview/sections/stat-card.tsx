    ...
    <Card className="p-6">
      <div className="flex flex-col">
        ...
        <div className="flex items-baseline mt-2">
          <span className="text-mid font-semibold">{value}</span>
          <div className={`ml-2 flex items-center ${trend === 'up' ? 'text-green-500' : 'text-red-500'}`}>
            ...
            <span className="text-small ml-1">{Math.abs(change)}%</span>
          </div>
        </div>
        <span className="text-small text-gray-500 mt-1">This month</span>
      </div>
    </Card>
  );
}