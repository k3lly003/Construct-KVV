...
export default function ArchitectPortfolioList({ title, description }: Props) {
  ...
  async function saveEdit() {
    ...
    async function compressToDataUrl(file: File, maxBytes = 100_000, maxDimension = 800, qualityStep = 0.6): Promise<string> {
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                    <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                      {p.skills?.[0]?.[0] ? p.skills[0][0].toUpperCase() : 'A'}
                    </span>
                  </div>
                </div>
