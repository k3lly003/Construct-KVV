...
export default function CommonPortfolioList({ professionalType, title, description }: Props) {
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent>
        {loading && <p className="text-small text-muted-foreground">Working...</p>}
        {!loading && items.length === 0 && (
          <p className="text-small text-muted-foreground">No public items yet.</p>
        )}
              ...
              {editingId === p.id ? (
                ...
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-small px-2 py-1 rounded ${p.isPublic ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{p.isPublic ? 'Public' : 'Private'}</span>
                  </div>
                  {p.images?.[0] && (
                    <img src={p.images[0]} alt={p.title} className="mt-1 h-32 w-full object-cover rounded" />
                  )}
                  <p className="text-small text-muted-foreground mt-1 line-clamp-3">{p.description}</p>
                  <div className="text-small text-gray-500">{p.location} • {formatPortfolioDate(p.workDate)}</div>
