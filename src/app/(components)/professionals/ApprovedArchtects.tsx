...
export default function ApprovedArchitects({ searchQuery = "" }: { searchQuery?: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.log('[ApprovedArchitects] fetched:', list.length, 'normalized:', normalized.length);
        setItems(normalized);
      } finally {
        setLoading(false);
      }
  ...
  console.log('[ApprovedArchitects] rendering count:', filtered.length, 'searchQuery:', searchQuery);
